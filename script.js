...
function render() {
    // 1. Capture current scroll position of the inspection container
    ...
    // 2. Restore scroll position if we are still in inspection view
                ...
                ${status ? `
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 pt-4 border-t animate-fade-in">
                        ...
                        <div class="md:col-span-3">
                            ...
                            <button onclick="updateItem('${item.id}', 'photos', ${(itemData.photos || 0) + 1})" class="w-full h-[80px] border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center text-gray-400 hover:text-blue-500 hover:border-blue-500 transition-colors">
                                <i data-lucide="camera" class="w-5 h-5 mb-1"></i>
                                <span class="text-xs font-medium">${itemData.photos ? `${itemData.photos} Added` : 'Add Photo'}</span>
                            </button>
                        </div>
            ...
            if (item.status === 'fail' || item.status === 'warning') {
                ...
                issuesHtml += `
                    <div class="flex justify-between items-start p-3 bg-white border rounded-lg shadow-sm break-inside-avoid">
                        ...
                        ${item.cost > 0 ? `<div class="text-right font-bold text-gray-800">$${parseFloat(item.cost).toFixed(2)}</div>` : ''}
                    </div>
                `;
            }
        });
...
// Input Binding (Event Delegation)
function bindEvents() {
    // We don't need to re-bind document level events, only specific element logic if needed.
    // Using delegation for inputs to avoid focus loss on re-render for simple inputs is tricky in vanilla HTML re-renders.
    // Solution: Inputs update state on input, but DO NOT re-render view. 
...
function updateItemData(catId, itemId, updates, shouldRender) {
    ...
    if (!state.inspectionData[catId][itemId]) state.inspectionData[catId][itemId] = { status: null, notes: '', photos: 0, cost: 0 };
